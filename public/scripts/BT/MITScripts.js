...
function saveVaultResult({ type, token, customerId, brand, last4 }) {
    if (type === "card") {
        ...
        if (threeDSAuthenticationId) localStorage.setItem("threeDSAuthenticationId", threeDSAuthenticationId); // NEW
        return;
    }
...
window.onload = async () => { 
    ...
    if (vault.vaulted) { //if vaulted status is true
        setTimeout(() => {
            window.location.href = `/html/${int}/MITReturnCust.html?int=${int}`;  // <-- returning customer
        }, 200); //
        return;
    }
            ...
            braintree.hostedFields.create({
                ...
                $('#cardForm').submit(function (event) {
                    ...
                    threeDS.verifyCard({
                        ...
                        // decide to block the payment
                        }
                        
                        // Required for 3RI later
                        const threeDSInfo = threeDSPayload.threeDSecureInfo || {};
                        const authenticationId = threeDSInfo.threeDSecureAuthenticationId;
                    ...
                    try{
                        ...
                        if (result.success) {
                            ...
                            saveVaultResult({
                                ...
                                threeDSAuthenticationId: authenticationId,
                            });
                            console.log(result);
                            alert("✅ Transaction successful!\nCustomer ID: " + result.customerId);    
                            setTimeout(() => {
                                window.location.href = `/html/${int}/MITReturnCust.html?int=${int}`;  // <-- exisiting customer
                            }, 2000); 
                        }
                        } catch (error){
