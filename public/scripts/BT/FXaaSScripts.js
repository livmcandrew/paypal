...
var setAmount = document.querySelector(".item-price").textContent.trim().replace("£", "");
let presentmentCurrency, settlementCurrency = 'GBP'; // default
var resultDiv = document.getElementById("divResponse");

//Handles currency selection dropdowns
document.addEventListener('DOMContentLoaded', () => {
  ...
  presentmentBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    presentmentDropdown.classList.toggle('show');
  });

  settlementBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    settlementDropdown.classList.toggle('show');
  });

  presentmentDropdown.querySelectorAll('.dropdown-item').forEach((item) => {
    item.addEventListener('click', (event) => {
      event.preventDefault();
      presentmentCurrency = item.dataset.currency;
      selectedPresentment.textContent = presentmentCurrency;
      presentmentDropdown.classList.remove('show');
      console.log('Presentment currency:', presentmentCurrency);
    });
  });

  settlementDropdown.querySelectorAll('.dropdown-item').forEach((item) => {
    item.addEventListener('click', (event) => {
      event.preventDefault();
      settlementCurrency = item.dataset.currency;
      selectedSettlement.textContent = settlementCurrency;
      settlementDropdown.classList.remove('show');
      console.log('Settlement currency:', settlementCurrency);
    });
  });

  document.addEventListener('click', (event) => {
    if (!presentmentDropdown.contains(event.target)) presentmentDropdown.classList.remove('show');
    if (!settlementDropdown.contains(event.target)) settlementDropdown.classList.remove('show');
  });
});
...
FXButton.addEventListener('click', async function (event) {
   ...
   try {
    const response = await fetch("/btcheckout/fx-quote", {
      ...
      body: JSON.stringify({
        baseCurrency: presentmentCurrency,
    ...
    console.log(quote);
    exchangeRate = quote[0].exchangeRate;
    //console.log(`Exchange Rate: ${exchangeRate}`);

    //show new price with exchange rate applied
    const newPrice = (setAmount * exchangeRate).toFixed(2);
    resultDiv.innerHTML = `Exchange Rate: ${exchangeRate} <br> New Price: ${newPrice} ${settlementCurrency}`;
